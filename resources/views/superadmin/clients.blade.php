...
@section('content')
        ...
        <div id="client-list" class="animate-fade-in">
                ...
                <div class="bg-white rounded-3xl shadow-sm overflow-hidden">
                    <div class="p-5 border-b border-gray-100 dark:border-slate-700">
                        <form method="GET" action="{{ route('superadmin.clients') }}" class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                            <!-- Titre à gauche -->
                            ...
                            <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                                ...
                                @if(request('search') || request('filter_type'))
                                    <a href="{{ route('superadmin.clients') }}" class="p-2 text-red-400 hover:text-red-600 transition" title="Réinitialiser">
                        ...
                        <div class="flex items-center gap-6">
                            ...
                            <div>
                                <div class="flex items-center gap-3 mb-2">
                                    <h2 class="text-3xl font-bold text-gray-800 dark:text-white" id="detail-name">Jean Dupont</h2>
                                ...
                                <p class="text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
                                    ...
                                    <span id="detail-phone">225 01 02 03 04</span>
                                </p>
                            </div>
                        </div>
                    ...
                    <div id="edit-form" class="hidden border-t border-gray-100 dark:border-slate-700 pt-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="edit-name" class="w-full px-4 py-2 bg-gray-50 dark:bg-slate-800 dark:text-white border border-gray-200 dark:border-slate-600 rounded-xl text-sm font-medium focus:ring-2 focus:ring-brand-blue/20 outline-none" value="Jean Dupont">
                            </div>
                            <div>
    ...
    <div id="delete-client-modal" class="hidden fixed inset-0 z-[110] flex items-center justify-center p-4">
        ...
        <div class="bg-white dark:bg-brand-cardDark w-full max-w-sm rounded-3xl p-6 relative z-10 text-center shadow-2xl border border-gray-100 dark:border-slate-700 animate-scale-in">
            ...
            <div class="flex gap-3">
                ...
                <button onclick="deleteClient()" class="flex-1 py-3 bg-red-500 text-white rounded-xl font-bold hover:red-600 transition shadow-lg">Confirmer</button>
            </div>
        </div>
    </div>
            ...
            <form action="#" method="POST" class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <div class="flex relative group">
                        ...
                        <div class="relative flex-1">
                            <input type="tel" name="telephone" id="add-client-phone" class="w-full px-4 py-3 bg-gray-50 dark:bg-slate-800 dark:text-white border border-gray-200 dark:border-slate-600 rounded-r-xl text-sm font-bold focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue outline-none transition placeholder-gray-400" placeholder="XX XX XX XX" required oninput="this.value = this.value.replace(/[^0-9]/g, '')">
                            <i class="fas fa-mobile-alt absolute right-4 top-3.5 text-gray-400"></i>
                        </div>
    ...
    <script>
        ...
        function showClientDetail(id, name, phone, zone, spent, isBlocked) {
            currentClientId = id;
            ...
            document.getElementById('detail-spent').textContent = spent.toLocaleString('fr-FR') + ' F';
        ...
        function updateBlockButtonUI(isBlocked) {
            ...
            if (isBlocked) {
                btn.textContent = 'DÉBLOQUER CE CLIENT';
                btn.className = 'px-4 py-2 bg-green-500 text-white rounded-xl text-xs font-bold transition';
            } else {
        ...
        function toggleBlock() {
            alert('Fonctionnalité de blocage/déblocage à implémenter');
        }
        ...
        function selectCountry(code, phoneCode, prefix) {
            document.getElementById(prefix + '-flag').src = 'https://flagcdn.com/w40/' + code + '.png';
            document.getElementById(prefix + '-code').textContent = phoneCode;
            document.getElementById(prefix + '-phone-code').value = phoneCode;
            document.getElementById(prefix + '-country-menu').classList.add('hidden');
        }
