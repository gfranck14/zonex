...
<body class="bg-brand-sidebarLight dark:bg-brand-sidebarDark text-slate-800 dark:text-slate-100 h-[100dvh] w-screen overflow-hidden flex transition-colors duration-300">
    ...
    <nav class="md:hidden fixed bottom-0 left-0 w-full bg-brand-sidebarLight dark:bg-brand-sidebarDark z-[100] pb-safe pt-2">
        <div class="flex items-center justify-around px-2 pb-3">
            <a href="{{ route('superadmin.dashboard') }}" class="flex flex-col items-center gap-1.5 min-w-[60px] {{ request()->routeIs('superadmin.dashboard') ? 'text-brand-blue drop-shadow-[0_0_8px_rgba(14,165,233,0.5)]' : 'text-gray-400 hover:text-white' }} transition-all">
                <i class="fas fa-th-large text-xl"></i>
                <span class="text-[10px] font-medium">Accueil</span>
            </a>
            <a href="{{ route('superadmin.proprietaires') }}" class="flex flex-col items-center gap-1.5 min-w-[60px] {{ request()->routeIs('superadmin.proprietaires') ? 'text-brand-blue drop-shadow-[0_0_8px_rgba(14,165,233,0.5)]' : 'text-gray-400 hover:text-white' }} transition-all">
                <i class="fas fa-building text-xl"></i>
                <span class="text-[10px] font-medium">Proprios</span>
            </a>
            <a href="{{ route('superadmin.clients') }}" class="flex flex-col items-center gap-1.5 min-w-[60px] {{ request()->routeIs('superadmin.clients') ? 'text-brand-blue drop-shadow-[0_0_8px_rgba(14,165,233,0.5)]' : 'text-gray-400 hover:text-white' }} transition-all">
                <i class="fas fa-users text-xl"></i>
                <span class="text-[10px] font-medium">Clients</span>
            </a>
            <a href="{{ route('superadmin.retraits') }}" class="flex flex-col items-center gap-1.5 min-w-[60px] {{ request()->routeIs('superadmin.retraits') ? 'text-brand-blue drop-shadow-[0_0_8px_rgba(14,165,233,0.5)]' : 'text-gray-400 hover:text-white' }} transition-all">
                <i class="fas fa-money-bill-wave text-xl"></i>
                <span class="text-[10px] font-medium">Retraits</span>
            </a>
        </div>
    </nav>
    ...
    <div id="pwa-install-modal" class="fixed inset-0 bg-black/60 z-[9999] hidden flex items-end sm:items-center justify-center p-4 transition-opacity duration-300">
        <div class="bg-white dark:bg-slate-800 w-full max-w-sm rounded-3xl p-6 shadow-2xl transform transition-transform duration-300 translate-y-full sm:translate-y-0" id="pwa-modal-content">
            <div class="flex flex-col items-center text-center">
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">Installez l'application SuperAdmin sur votre écran d'accueil pour un accès rapide.</p>
            </div>
